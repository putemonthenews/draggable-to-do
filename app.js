...
function createDragableTarget(node) {
  ...
  node.addEventListener("dragover", function (evt) {
    ...
    if (onDragItem.parentElement.parentElement.dataset.todoName === 'incomplete' || onDragItem.parentElement.parentElement.dataset.todoName === 'trash') {
  ...
  const timerSeconds = document.querySelectorAll('.todo__seconds')
    ...
    timerSeconds.forEach((timerSecond, idx) => {
      ...
      if (hour.textContent == 0 && minute.textContent == 0 && timerSecond.textContent == 0) {
        hour.textContent = 'Время закончилось'
        ...
        hour.parentElement.querySelector('.indicator').style.backgroundColor = '#ef4444'
...
const incompleteTimes = incompleteCard.querySelectorAll('.remainingTime')
